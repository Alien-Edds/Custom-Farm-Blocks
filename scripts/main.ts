...
world.beforeEvents.playerBreakBlock.subscribe((data) => {
    ...
    if (!block.hasTag("example:disable_silk_touch")) return
    if (!itemStack) return
    const comp = itemStack.getComponent(ItemEnchantableComponent.componentId) as ItemEnchantableComponent | undefined
    if (!comp) return
